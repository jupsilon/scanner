...
(defun emit-client-listener-handler-binder (events)
  (indent/<<
    (dolist (event events)
  ...
  (let* ((name         (node-get-value event "name"))
	...
	(let ((result-type    (elt result 0))
      ...
      (emit-lines notes
		  "/// @note    "
		  "///          ")
      (<<         "virtual " (emit-result-type result) " " name (emit-formal-parameters-list formal-list) " const;")
      (<<)
      (<<         "/// @brief   delegation for " name)
      (<<         "delegation<" (emit-result-type result) " " (emit-parameter-types-list types-list) "> " name "_delegation;"))))
  ...
  (let ((requests (nodes-select interface-members "request"))
	...
	(indent/<<
	  ...
	  (indent/<<
	    ...
	    (emit-client-listener-handler-binder events)
	    (<<     "};"))
	  (<<     "}"))
	(<<)
	(<<     "public:")
...
(defun params-actual-arguments-list (params)
  (mapcar (lambda (param) (elt param 1)) params))
